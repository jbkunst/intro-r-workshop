...
<div id="left">
![import](static/imgs/filter.png) 

<br>

![import](static/imgs/select.png) 
</div>
<div id="right">
![import](static/imgs/group_by_summarize.png) 

<br>

![import](static/imgs/mutate.png) 
</div>
...
![Flujo de trabajo](static/imgs/data-viz.jpg) 
...
<!-- Obtener las comunas que no -->

<!-- ```{r} -->
<!-- dataaug <- datag %>%  -->
<!--   mutate(augment = map(modelo, augment)) %>% -->
<!--   select(region2, data, augment) %>%  -->
<!--   unnest() -->
<!-- glimpse(dataaug) -->
<!-- ``` -->
...
<!-- dataaugmin <- dataaug %>%  -->
<!--   group_by(region2) %>%  -->
<!--   arrange(region2, desc(abs(.std.resid))) %>%  -->
<!--   top_n(2) -->

<!-- ggplot(dataaugmin) + -->
<!--   geom_col(aes(COMUNA, abs(.std.resid))) + -->
<!--   coord_flip() + -->
